//! Editable code area with syntax highlighting (transparent textarea over a coloured pre).
...
pub fn editor(props: &EditorProps) -> Html {
    ...
    let container_style = "\
        ...
        overflow:hidden; background:#181825; display:flex;";

    let gutter_style = format!(
        "width:{gutter_width}; min-width:{gutter_width}; \
         background:#11111b; color:#a6adc8; \
         font-family:'SF Mono','Fira Code',monospace; font-size:14px; line-height:1.5; \
         padding:12px 8px 12px 0; text-align:right; user-select:none; overflow:hidden; \
         border-right:1px solid #313244; box-sizing:border-box;"
    );

    let text_style = "\
        font-family:'SF Mono','Fira Code',monospace; font-size:14px; line-height:1.5; \
        padding:12px; tab-size:4; white-space:pre;";

    let edit_area_style = "position:relative; flex:1; min-width:0;";

    let pre_style = format!(
        "{text_style} position:absolute; top:0; left:0; right:0; bottom:0; \
         margin:0; overflow:auto; pointer-events:none; color:#cdd6f4;"
    );

    let textarea_style = format!(
        "{text_style} position:absolute; top:0; left:0; width:100%; height:100%; \
    ...
    html! {
        <div style={container_style}>
            ...
            <div style={edit_area_style}>
                <pre ref={pre_ref} style={pre_style}><code>{highlighted}</code></pre>
                <textarea
